...
class Waypoint {
  ...
  delay(typicalDelay, waypointDelays) {
    ...
    const wpd = waypointDelays.find((wpd) => wpd.site === this.site._id && this.loop === wpd.loop)
    if (wpd) {
      return wpd.delay
  ...
  matchingSegment(segments) {
    return segments.find(
      (s) => s.waypoint.site._id === this.site._id && s.waypoint.loop === this.loop
    )
  }
